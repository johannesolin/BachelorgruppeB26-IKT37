...
const tokenProvider_1 = require("./connection/auth/tokenProvider");
...
class DBSQLClient extends events_1.EventEmitter {
    ...
    createAuthProvider(options, authProvider) {
        ...
        switch (options.authType) {
            ...
            case 'token-provider':
                return new tokenProvider_1.TokenProviderAuthenticator(this.wrapTokenProvider(options.tokenProvider, options.host, options.enableTokenFederation, options.federationClientId), this);
            case 'external-token':
                return new tokenProvider_1.TokenProviderAuthenticator(this.wrapTokenProvider(new tokenProvider_1.ExternalTokenProvider(options.getToken), options.host, options.enableTokenFederation, options.federationClientId), this);
            case 'static-token':
                return new tokenProvider_1.TokenProviderAuthenticator(this.wrapTokenProvider(tokenProvider_1.StaticTokenProvider.fromJWT(options.staticToken), options.host, options.enableTokenFederation, options.federationClientId), this);
            // no default
        }
    }
    /**
     ...
     */
    wrapTokenProvider(provider, host, enableFederation, federationClientId) {
        // Always wrap with caching first
        let wrapped = new tokenProvider_1.CachedTokenProvider(provider);
        // Optionally wrap with federation
        if (enableFederation) {
            wrapped = new tokenProvider_1.FederationProvider(wrapped, host, {
